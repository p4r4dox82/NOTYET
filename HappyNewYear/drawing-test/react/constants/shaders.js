...
export const NoiseShader = {
  ...
  fragmentShader: `
    ...
    float fbm(vec3 x) {
      float v = 0.0;
      float a = 0.5; 
      vec3 shift = vec3(10.0); // 겹칠 때 위치를 살짝 비틀어줌 (자연스러움 상승)
    ...
    void main() {
      ...
      vec3 coord = vec3(vUv * 10.0, uTime * 0.2); 
      
      // 2. 오직 snoise 딱 한 번만 실행! (fbm 안 씀)
      float n = snoise(coord);
      
      // 3. snoise는 -1 ~ 1 사이로 나오므로, 화면에 보이게 0 ~ 1로 부드럽게 맵핑
